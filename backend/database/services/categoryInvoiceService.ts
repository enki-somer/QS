...
import { Invoice, InvoiceLineItem, ProjectCategoryAssignment } from '../types';

export interface CategoryInvoiceData {
...
export interface CategoryInvoiceResponse extends Invoice {
  ...
  categoryName: string;
  subcategoryName: string;
  assignmentDetails: ProjectCategoryAssignment;
}
...
class CategoryInvoiceService {
   ...
   * This links an invoice to a specific project category assignment
   */
  async createCategoryInvoice(data: CategoryInvoiceData, createdBy?: string): Promise<CategoryInvoiceResponse> {
    ...
    try {
      ...
      // If no createdBy provided, we still need a valid UUID for the database
      if (!createdBy) {
      ...
      // First, verify the category assignment exists and get its details
      ...
      // Allow multiple invoices per assignment for progressive billing
      // The frontend should handle budget tracking and prevent over-billing
      ...
      const invoiceValues = [
        ...
        data.invoiceNumber,
        data.amount,
        data.amount, // subtotal = amount for category invoices
        data.date,
        data.notes,
        'pending_approval',
        createdBy
      ...
      // Create a single line item for the category work
      const lineItemQuery = `
      ...
      const lineItemValues = [
        ...
        data.description,
        1, // quantity
        data.amount, // unit price
        data.amount // total
      ];

      await client.query(lineItemQuery, lineItemValues);

      // Update the category assignment with pending invoice info
   ...
   */
  async getCategoryInvoices(categoryAssignmentId: string): Promise<CategoryInvoiceResponse[]> {
    const query = `
      SELECT 
        i.*,
    ...
    const result = await getPool().query(query, [categoryAssignmentId]);
   ...
   */
  async getInvoicesByProjectAndCategory(projectId: string, categoryName: string): Promise<any[]> {
    // Map English category IDs to Arabic names
   ...
   */
  async getInvoiceCountsByProject(projectId: string): Promise<Record<string, number>> {
    // Map Arabic category names to English IDs
  ...
  /**
   * Get category invoice status for all assignments in a project
   */
  async getProjectCategoryInvoiceStatus(projectId: string) {
    const query = `
      SELECT 
  ...
  /**
   * Approve a category invoice (this will trigger the financial protection)
   */
  async approveCategoryInvoice(invoiceId: string, approvedBy: string): Promise<void> {
    const client = await getPool().connect();
...
export default new CategoryInvoiceService();