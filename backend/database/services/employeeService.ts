import { Pool } from 'pg';
import { DatabaseResult, Employee, SalaryPaymentData } from '../types';
...
class EmployeeService {
   ...
   */
  async createEmployee(data: CreateEmployeeData): Promise<DatabaseResult<Employee>> {
   ...
   */
  async getEmployees(filter?: EmployeeFilter): Promise<DatabaseResult<Employee[]>> {
    const client = await this.pool.connect();
    try {
      let query = `
   ...
   * Get employee by ID
   */
  async getEmployeeById(id: string): Promise<DatabaseResult<Employee>> {
    const client = await this.pool.connect();
    try {
      const query = `
        SELECT e.*, p.name as project_name
   ...
   * Update employee
   */
  async updateEmployee(id: string, data: UpdateEmployeeData): Promise<DatabaseResult<Employee>> {
    const client = await this.pool.connect();
    try {
   ...
   */
  async processSalaryPayment(employeeId: string, paymentData: SalaryPaymentData): Promise<DatabaseResult<any>> {
    const client = await this.pool.connect();
    try {
      ...
      // Insert payment record with enhanced fields
      const paymentQuery = `
      ...
      // Update employee last payment date and payment status
   ...
   */
  async getEmployeePaymentHistory(employeeId: string): Promise<DatabaseResult<any[]>> {
    const client = await this.pool.connect();
    try {
      const query = `
   ...
   */
  async calculateRemainingSalary(employeeId: string): Promise<DatabaseResult<{ remainingSalary: number, totalPaidThisMonth: number, monthlySalary: number }>> {
    const client = await this.pool.connect();
    try {
   ...
   */
  async getPositions(): Promise<DatabaseResult<any[]>> {
    const client = await this.pool.connect();
    try {
      const query = `
  ...
  /**
   * Get all projects for dropdown (view-only reference)
   */
  ...
  /**
   * Get employees due for payment (simple version)
   */
